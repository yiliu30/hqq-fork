...
if DEVICE == "cpu":
    if qbits_available:
        ...
        raise ImportError(
            "Please import intel-extension-for-transformers " "by `pip install intel-extension-for-transformers`"
        )
else:
...
def _load_model(model_path, no_safetensors, pretrained, quant_file, n_generate, batch_size):
    if pretrained:
        ...
        model = AutoAWQForCausalLM.from_quantized(
            model_path,
            quant_file,
            fuse_layers=False if DEVICE == "cpu" else True,
...
def run_round(
    generator, model_path, quant_file, n_generate, input_ids, batch_size, no_safetensors, pretrained, model=None
):
    print(f" -- Loading model...")
    if model is None:
        model = _load_model(model_path, no_safetensors, pretrained, quant_file, n_generate, batch_size)
...
def benchmodel(
    ...
    """
    Usage examples:
        benchmodel(loaded_model)
    """
    if generator_name == "torch":
        generator = generate_torch
    elif generator_name == "hf":
        generator = generate_hf
    else:
        raise ValueError(f"Unknown generator method passed: {generator_name}")
...
def main(args):
    benchmodel(args.generator, args.model_path, args.quant_file, args.batch_size, args.no_safetensors, args.pretrained)
